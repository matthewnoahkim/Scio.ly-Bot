...
for (const filePath of commandFiles) {
	try {
		...
		console.error(`[ERROR] Failed to load command from ${filePath}:`, (error as Error).message);
	}
}
...
const rest = new REST({ timeout: 60_000 }).setToken(token);
...
void (async () => {
	try {
		...
		const data = (await rest.put(Routes.applicationCommands(clientId), { body: commands })) as Array<{ name: string }>;

		console.log(`✅ Successfully reloaded ${data.length} application (/) commands.`);
		console.log('Deployed commands:', data.map(c => c.name).join(', '));
	}
	catch (error) {
		const err = error as { code?: string; message?: string; status?: number; response?: { data?: unknown } };
