...
if (!API_KEY) {
  console.warn('[designergenes] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
const subtopicOptions = ["Genetics", "DNA", "Proteins", "Evolution", "Population Genetics"];
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[designergenes] Could not resolve correct index from answers:', answers);
  return 0;
}
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Get a Designer Genes question')
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[designergenes] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[designergenes] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[designergenes] Starting FRQ grading for:', {
                ...
                console.log('[designergenes] Prepared grading request:', {
                ...
                console.log('[designergenes] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[designergenes] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[designergenes] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[designergenes] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[designergenes] Trying fallback API...');
                    ...
                    console.log('[designergenes] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[designergenes] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[designergenes] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[designergenes] No score or grade found in response, showing error to user');
                  console.log('[designergenes] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[designergenes] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[designergenes] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Designer Genes',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[designergenes] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[designergenes] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[designergenes] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Designer Genes',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[designergenes] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[designergenes] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[designergenes] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[designergenes] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[designergenes] Button handler error:', innerErr);
          try {
