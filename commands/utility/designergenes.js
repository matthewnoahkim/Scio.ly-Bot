          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            if (isMCQ) {
              ...
              try {
                ...
                console.error('[designergenes] FRQ grading error:', err?.response?.status, err?.message);
            ...
            try {
              const explanation = await getExplanationWithRetry(question, 'Designer Genes', AUTH_HEADERS, 'designergenes');
              ...
              console.error('[designergenes] Explanation error:', err?.response?.status, err?.message);
              if (err?.response?.status === 429) {
