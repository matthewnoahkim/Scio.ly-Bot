...
const subtopicOptions = ["Solar System", "Stars", "Galaxies", "Cosmology", "Instruments"];
...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[astronomy] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[astronomy] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[astronomy] Starting FRQ grading for:', {
                ...
                console.log('[astronomy] Prepared grading request:', {
                ...
                console.log('[astronomy] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[astronomy] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[astronomy] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[astronomy] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[astronomy] Trying fallback API...');
                    ...
                    console.log('[astronomy] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[astronomy] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[astronomy] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[astronomy] No score or grade found in response, showing error to user');
                  console.log('[astronomy] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[astronomy] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[astronomy] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Astronomy',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[astronomy] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[astronomy] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[astronomy] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Astronomy',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[astronomy] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[astronomy] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[astronomy] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[astronomy] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
