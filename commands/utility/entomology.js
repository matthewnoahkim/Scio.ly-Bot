...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('entomology')
    .setDescription('Get an Entomology question')
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            if (isMCQ) {
              ...
              try {
                ...
                console.error('[entomology] FRQ grading error:', err?.response?.status, err?.message);
            ...
            try {
              ...
              console.error('[entomology] Explanation error:', err?.response?.status, err?.message);
              if (err?.response?.status === 429) {
