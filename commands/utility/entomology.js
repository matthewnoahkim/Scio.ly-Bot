...
if (!API_KEY) {
  console.warn('[entomology] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
const subtopicOptions = ["Insect Anatomy", "Life Cycles", "Behavior", "Classification", "Ecology"];
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[entomology] Could not resolve correct index from answers:', answers);
  return 0;
}
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('entomology')
    .setDescription('Get a Entomology question')
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[entomology] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[entomology] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[entomology] Starting FRQ grading for:', {
                ...
                console.log('[entomology] Prepared grading request:', {
                ...
                console.log('[entomology] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[entomology] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[entomology] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[entomology] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[entomology] Trying fallback API...');
                    ...
                    console.log('[entomology] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[entomology] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[entomology] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[entomology] No score or grade found in response, showing error to user');
                  console.log('[entomology] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[entomology] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[entomology] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Entomology',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[entomology] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[entomology] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[entomology] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Entomology',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[entomology] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[entomology] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[entomology] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[entomology] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[entomology] Button handler error:', innerErr);
          try {
      ...
      console.error('Error in Entomology command:', err);
