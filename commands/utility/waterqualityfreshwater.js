...
const subtopicOptions = ["Physical Properties", "Dissolved Oxygen", "Nutrients", "Pollutants", "Testing"];
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            if (isMCQ) {
              ...
              try {
                ...
                console.error('[waterqualityfreshwater] FRQ grading error:', err?.response?.status, err?.message);
            ...
            try {
              const explanation = await getExplanationWithRetry(question, 'Water Quality - Freshwater', AUTH_HEADERS, 'waterqualityfreshwater');
              ...
              console.error('[waterqualityfreshwater] Explanation error:', err?.response?.status, err?.message);
              if (err?.response?.status === 429) {
