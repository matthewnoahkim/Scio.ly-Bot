// /commands/waterqualityfreshwater.js
const { 
  SlashCommandBuilder, 
  EmbedBuilder, 
  ActionRowBuilder, 
...
if (!API_KEY) {
  console.warn('[waterqualityfreshwater] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
const subtopicOptions = ["PH", "Dissolved Oxygen", "Nutrients", "Pollutants", "Testing"];
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[waterqualityfreshwater] Could not resolve correct index from answers:', answers);
  return 0;
}
...
function buildQuestionEmbed(question) {
  const embed = new EmbedBuilder()
    .setColor(0x0099ff)
    .setTitle('Water Quality - Freshwater')
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('waterqualityfreshwater')
    .setDescription('Get a Water Quality - Freshwater question')
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[waterqualityfreshwater] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[waterqualityfreshwater] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[waterqualityfreshwater] Starting FRQ grading for:', {
                ...
                console.log('[waterqualityfreshwater] Prepared grading request:', {
                ...
                console.log('[waterqualityfreshwater] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[waterqualityfreshwater] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[waterqualityfreshwater] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[waterqualityfreshwater] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[waterqualityfreshwater] Trying fallback API...');
                    ...
                    console.log('[waterqualityfreshwater] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[waterqualityfreshwater] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[waterqualityfreshwater] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[waterqualityfreshwater] No score or grade found in response, showing error to user');
                  console.log('[waterqualityfreshwater] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[waterqualityfreshwater] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[waterqualityfreshwater] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Water Quality - Freshwater',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[waterqualityfreshwater] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[waterqualityfreshwater] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[waterqualityfreshwater] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Water Quality - Freshwater',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[waterqualityfreshwater] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[waterqualityfreshwater] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[waterqualityfreshwater] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[waterqualityfreshwater] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[waterqualityfreshwater] Button handler error:', innerErr);
          try {
      ...
      console.error('Error in Water Quality - Freshwater command:', err);
