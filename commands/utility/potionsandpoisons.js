          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            if (isMCQ) {
              ...
              try {
                ...
                console.error('[potionsandpoisons] FRQ grading error:', err?.response?.status, err?.message);
            ...
            try {
              const explanation = await getExplanationWithRetry(question, 'Potions and Poisons', AUTH_HEADERS, 'potionsandpoisons');
              ...
              console.error('[potionsandpoisons] Explanation error:', err?.response?.status, err?.message);
              if (err?.response?.status === 429) {
