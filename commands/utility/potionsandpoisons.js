...
if (!API_KEY) {
  console.warn('[potionsandpoisons] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
const subtopicOptions = ["Toxicology", "Pharmacology", "Dosage", "Symptoms", "Antidotes"];
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[potionsandpoisons] Could not resolve correct index from answers:', answers);
  return 0;
}
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('potionsandpoisons')
    .setDescription('Get a Potions and Poisons question')
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[potionsandpoisons] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[potionsandpoisons] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[potionsandpoisons] Starting FRQ grading for:', {
                ...
                console.log('[potionsandpoisons] Prepared grading request:', {
                ...
                console.log('[potionsandpoisons] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[potionsandpoisons] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[potionsandpoisons] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[potionsandpoisons] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[potionsandpoisons] Trying fallback API...');
                    ...
                    console.log('[potionsandpoisons] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[potionsandpoisons] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[potionsandpoisons] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[potionsandpoisons] No score or grade found in response, showing error to user');
                  console.log('[potionsandpoisons] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[potionsandpoisons] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[potionsandpoisons] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Potions and Poisons',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[potionsandpoisons] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[potionsandpoisons] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[potionsandpoisons] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Potions and Poisons',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[potionsandpoisons] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[potionsandpoisons] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[potionsandpoisons] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[potionsandpoisons] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[potionsandpoisons] Button handler error:', innerErr);
          try {
      ...
      console.error('Error in Potions and Poisons command:', err);
