...
// ----- helpers -----
function prune(obj){ return Object.fromEntries(Object.entries(obj).filter(([,v])=>v!==undefined && v!==null)); }
function normalize(t){ return String(t ?? '').trim().replace(/\s+/g, ' ').toLowerCase(); }
...
function buildQuestionEmbed(q, isID){
  const e = new EmbedBuilder()
    .setColor(COLOR_BLUE)
    .setTitle(EVENT_NAME)
    .setDescription(q.question || 'No question text');

  const fields = [];
  if (Array.isArray(q.options) && q.options.length) {
    fields.push({
      name: 'Answer Choices',
      value: q.options.map((opt,i)=>`**${letterFromIndex(i)})** ${opt}`).join('\n'),
      inline: false
    });
  }
  fields.push(
...
module.exports = {
  ...
  async execute(interaction){
    try {
      ...
      const difficulty_min = difficultyLabel ? difficultyMap[difficultyLabel]?.min : undefined;
      const difficulty_max = difficultyLabel ? difficultyMap[difficultyLabel]?.max : undefined;
      ...
      if (isID) {
        ...
        const res = await axios.get(`${PRIMARY_BASE}/api/id-questions`, {
          params, timeout: 15000, headers: AUTH_HEADERS
        });
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          try { if (!btn.replied && !btn.deferred) await btn.reply('Something went wrong handling that action.'); } catch {}
        }
      });
