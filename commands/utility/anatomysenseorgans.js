...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Get an Anatomy - Sense Organs question')
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            if (isMCQ) {
              ...
              try {
                ...
                console.error('[anatomysenseorgans] FRQ grading error:', err?.response?.status, err?.message);
            ...
            try {
              const explanation = await getExplanationWithRetry(question, 'Anatomy - Sense Organs', AUTH_HEADERS, 'anatomysenseorgans');
              ...
              console.error('[anatomysenseorgans] Explanation error:', err?.response?.status, err?.message);
              if (err?.response?.status === 429) {
