// /commands/anatomysenseorgans.js
const { 
  SlashCommandBuilder, 
  EmbedBuilder, 
  ActionRowBuilder, 
...
if (!API_KEY) {
  console.warn('[anatomysenseorgans] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
const subtopicOptions = ["Eyes", "Ears", "Nose", "Tongue", "Skin"];
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[anatomysenseorgans] Could not resolve correct index from answers:', answers);
  return 0;
}
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('anatomysenseorgans')
    .setDescription('Get a Anatomy - Sense Organs question')
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[anatomysenseorgans] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[anatomysenseorgans] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[anatomysenseorgans] Starting FRQ grading for:', {
                ...
                console.log('[anatomysenseorgans] Prepared grading request:', {
                ...
                console.log('[anatomysenseorgans] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[anatomysenseorgans] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[anatomysenseorgans] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[anatomysenseorgans] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[anatomysenseorgans] Trying fallback API...');
                    ...
                    console.log('[anatomysenseorgans] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[anatomysenseorgans] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[anatomysenseorgans] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[anatomysenseorgans] No score or grade found in response, showing error to user');
                  console.log('[anatomysenseorgans] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[anatomysenseorgans] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[anatomysenseorgans] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Anatomy - Sense Organs',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[anatomysenseorgans] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[anatomysenseorgans] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[anatomysenseorgans] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Anatomy - Sense Organs',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[anatomysenseorgans] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[anatomysenseorgans] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[anatomysenseorgans] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[anatomysenseorgans] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[anatomysenseorgans] Button handler error:', innerErr);
          try {
      ...
      console.error('Error in Anatomy - Sense Organs command:', err);
