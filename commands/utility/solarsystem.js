        ...
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[solarsystem] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Solar System',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[solarsystem] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[solarsystem] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[solarsystem] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Solar System',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[solarsystem] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[solarsystem] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[solarsystem] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
