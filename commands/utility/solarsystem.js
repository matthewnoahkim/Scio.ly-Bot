          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            if (isMCQ) {
              ...
              try {
                ...
                console.error('[solarsystem] FRQ grading error:', err?.response?.status, err?.message);
            ...
            try {
              const explanation = await getExplanationWithRetry(question, 'Solar System', AUTH_HEADERS, 'solarsystem');
              ...
              console.error('[solarsystem] Explanation error:', err?.response?.status, err?.message);
              if (err?.response?.status === 429) {
