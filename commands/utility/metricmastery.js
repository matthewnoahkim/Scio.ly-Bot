          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            if (isMCQ) {
              ...
              try {
                ...
                console.error('[metricmastery] FRQ grading error:', err?.response?.status, err?.message);
            ...
            try {
              const explanation = await getExplanationWithRetry(question, 'Metric Mastery', AUTH_HEADERS, 'metricmastery');
              ...
              console.error('[metricmastery] Explanation error:', err?.response?.status, err?.message);
              if (err?.response?.status === 429) {
