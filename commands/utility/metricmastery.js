...
if (!API_KEY) {
  console.warn('[metricmastery] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
const subtopicOptions = ["Estimation", "Orders of Magnitude", "Problem Solving", "Scientific Reasoning", "Calculations"];
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[metricmastery] Could not resolve correct index from answers:', answers);
  return 0;
}
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('metricmastery')
    .setDescription('Get a Metric Mastery question')
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[metricmastery] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[metricmastery] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[metricmastery] Starting FRQ grading for:', {
                ...
                console.log('[metricmastery] Prepared grading request:', {
                ...
                console.log('[metricmastery] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[metricmastery] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[metricmastery] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[metricmastery] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[metricmastery] Trying fallback API...');
                    ...
                    console.log('[metricmastery] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[metricmastery] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[metricmastery] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[metricmastery] No score or grade found in response, showing error to user');
                  console.log('[metricmastery] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[metricmastery] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[metricmastery] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Metric Mastery',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[metricmastery] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[metricmastery] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[metricmastery] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Metric Mastery',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[metricmastery] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[metricmastery] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[metricmastery] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[metricmastery] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[metricmastery] Button handler error:', innerErr);
          try {
      ...
      console.error('Error in Metric Mastery command:', err);
