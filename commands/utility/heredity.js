...
if (!API_KEY) {
  console.warn('[heredity] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[heredity] Could not resolve correct index from answers:', answers);
  return 0;
}
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('heredity')
    .setDescription('Get a Heredity question')
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[heredity] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[heredity] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[heredity] Starting FRQ grading for:', {
                ...
                console.log('[heredity] Prepared grading request:', {
                ...
                console.log('[heredity] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[heredity] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[heredity] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[heredity] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[heredity] Trying fallback API...');
                    ...
                    console.log('[heredity] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[heredity] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[heredity] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[heredity] No score or grade found in response, showing error to user');
                  console.log('[heredity] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[heredity] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[heredity] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Heredity',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[heredity] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[heredity] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[heredity] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Heredity',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[heredity] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[heredity] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[heredity] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[heredity] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[heredity] Button handler error:', innerErr);
          try {
      ...
      console.error('Error in Heredity command:', err);
