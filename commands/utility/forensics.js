...
if (!API_KEY) {
  console.warn('[forensics] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
const subtopicOptions = ["Evidence Analysis", "Fingerprints", "DNA", "Toxicology", "Crime Scene"];
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[forensics] Could not resolve correct index from answers:', answers);
  return 0;
}
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('forensics')
    .setDescription('Get a Forensics question')
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[forensics] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[forensics] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[forensics] Starting FRQ grading for:', {
                ...
                console.log('[forensics] Prepared grading request:', {
                ...
                console.log('[forensics] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[forensics] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[forensics] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[forensics] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[forensics] Trying fallback API...');
                    ...
                    console.log('[forensics] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[forensics] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[forensics] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[forensics] No score or grade found in response, showing error to user');
                  console.log('[forensics] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[forensics] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[forensics] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Forensics',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[forensics] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[forensics] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[forensics] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Forensics',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[forensics] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[forensics] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[forensics] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[forensics] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[forensics] Button handler error:', innerErr);
          try {
      ...
      console.error('Error in Forensics command:', err);
