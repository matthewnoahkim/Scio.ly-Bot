          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            if (isMCQ) {
              ...
              try {
                ...
                console.error('[anatomyendocrine] FRQ grading error:', err?.response?.status, err?.message);
            ...
            try {
              const explanation = await getExplanationWithRetry(question, EVENT_NAME, AUTH_HEADERS, 'anatomyendocrine');
              ...
              console.error('[anatomyendocrine] Explanation error:', err?.response?.status, err?.message);
              if (err?.response?.status === 429) {
