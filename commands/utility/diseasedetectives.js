...
if (!API_KEY) {
  console.warn('[diseasedetectives] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
const subtopicOptions = ["Epidemiology", "Pathogens", "Prevention", "Outbreak Investigation", "Statistics"];
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[diseasedetectives] Could not resolve correct index from answers:', answers);
  return 0;
}
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('diseasedetectives')
    .setDescription('Get a Disease Detectives question')
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[diseasedetectives] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[diseasedetectives] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[diseasedetectives] Starting FRQ grading for:', {
                ...
                console.log('[diseasedetectives] Prepared grading request:', {
                ...
                console.log('[diseasedetectives] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[diseasedetectives] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[diseasedetectives] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[diseasedetectives] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[diseasedetectives] Trying fallback API...');
                    ...
                    console.log('[diseasedetectives] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[diseasedetectives] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[diseasedetectives] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[diseasedetectives] No score or grade found in response, showing error to user');
                  console.log('[diseasedetectives] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[diseasedetectives] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[diseasedetectives] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Disease Detectives',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[diseasedetectives] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[diseasedetectives] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[diseasedetectives] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Disease Detectives',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[diseasedetectives] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[diseasedetectives] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[diseasedetectives] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[diseasedetectives] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[diseasedetectives] Button handler error:', innerErr);
          try {
      ...
      console.error('Error in Disease Detectives command:', err);
