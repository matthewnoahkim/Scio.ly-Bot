// /commands/dynamicplanetoceanography.js
const { 
  SlashCommandBuilder, 
  EmbedBuilder, 
  ActionRowBuilder, 
...
if (!API_KEY) {
  console.warn('[dynamicplanetoceanography] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
const subtopicOptions = ["Ocean Circulation", "Marine Life", "Chemistry", "Geology", "Climate"];
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[dynamicplanetoceanography] Could not resolve correct index from answers:', answers);
  return 0;
}
...
function buildQuestionEmbed(question) {
  const embed = new EmbedBuilder()
    .setColor(0x0099ff)
    .setTitle('Dynamic Planet - Oceanography')
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('dynamicplanetoceanography')
    .setDescription('Get a Dynamic Planet - Oceanography question')
  ...
  async execute(interaction) {
    try {
      ...
      const baseParams = prune({
        event: 'Dynamic Planet - Oceanography',
        division,
        difficulty_min,
        difficulty_max,
        subtopic,
      ...
      console.log('[dynamicplanetoceanography] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[dynamicplanetoceanography] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[dynamicplanetoceanography] Starting FRQ grading for:', {
                ...
                console.log('[dynamicplanetoceanography] Prepared grading request:', {
                ...
                console.log('[dynamicplanetoceanography] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[dynamicplanetoceanography] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[dynamicplanetoceanography] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[dynamicplanetoceanography] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[dynamicplanetoceanography] Trying fallback API...');
                    ...
                    console.log('[dynamicplanetoceanography] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[dynamicplanetoceanography] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[dynamicplanetoceanography] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[dynamicplanetoceanography] No score or grade found in response, showing error to user');
                  console.log('[dynamicplanetoceanography] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[dynamicplanetoceanography] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[dynamicplanetoceanography] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Dynamic Planet - Oceanography',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[dynamicplanetoceanography] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[dynamicplanetoceanography] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[dynamicplanetoceanography] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Dynamic Planet - Oceanography',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[dynamicplanetoceanography] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[dynamicplanetoceanography] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[dynamicplanetoceanography] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[dynamicplanetoceanography] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[dynamicplanetoceanography] Button handler error:', innerErr);
          try {
      ...
      console.error('Error in Dynamic Planet - Oceanography command:', err);
