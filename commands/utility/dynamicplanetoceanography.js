          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            if (isMCQ) {
              ...
              try {
                ...
                console.error('[dynamicplanetoceanography] FRQ grading error:', err?.response?.status, err?.message);
            ...
            try {
              const explanation = await getExplanationWithRetry(question, 'Dynamic Planet - Oceanography', AUTH_HEADERS, 'dynamicplanetoceanography');
              ...
              console.error('[dynamicplanetoceanography] Explanation error:', err?.response?.status, err?.message);
              if (err?.response?.status === 429) {
