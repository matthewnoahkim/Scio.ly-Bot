...
if (!API_KEY) {
  console.warn('[chemistrylab] No SCIO_API_KEY found in environment variables. API calls may fail.');
}
...
const subtopicOptions = ["Stoichiometry", "Equilibrium", "Periodicity", "Redox Reactions", "Aqueous Solutions"];
...
function resolveCorrectIndex(question) {
  ...
  console.warn('[chemistrylab] Could not resolve correct index from answers:', answers);
  return 0;
}
...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      console.log('[chemistrylab] Question loaded:', {
        id: question.id,
      ...
      collector.on('collect', async (btn) => {
        try {
          ...
          if (btn.customId === `check_${question.id || interaction.id}`) {
            ...
            console.log('[chemistrylab] Check button clicked:', {
              isMCQ,
              hasOptions: Array.isArray(question.options) && question.options.length > 0,
            ...
            if (isMCQ) {
              ...
              // FRQ grading
              try {
                console.log('[chemistrylab] Starting FRQ grading for:', {
                ...
                console.log('[chemistrylab] Prepared grading request:', {
                ...
                console.log('[chemistrylab] Request body:', JSON.stringify(requestBody, null, 2));
                
                // Try the primary API first
                let gradeRes;
                try {
                  console.log('[chemistrylab] Trying primary grading API...');
                  gradeRes = await axios.post(`${PRIMARY_BASE}/api/gemini/grade-free-responses`, requestBody, { headers: AUTH_HEADERS });
                  console.log('[chemistrylab] Primary grading API success, response structure:', Object.keys(gradeRes.data));
                } catch (primaryErr) {
                  console.log('[chemistrylab] Primary grading API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                  
                  // Try fallback API with same format
                  try {
                    console.log('[chemistrylab] Trying fallback API...');
                    ...
                    console.log('[chemistrylab] Fallback grading API success, response structure:', Object.keys(gradeRes.data));
                  } catch (fallbackErr) {
                    console.log('[chemistrylab] Fallback grading API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                    throw fallbackErr; // Re-throw to be caught by outer catch
                  }
                }

                console.log('[chemistrylab] Grading API response received:', {
                ...
                if (grade) {
                  ...
                  console.log('[chemistrylab] No score or grade found in response, showing error to user');
                  console.log('[chemistrylab] Available data keys:', Object.keys(gradeRes.data?.data || {}));
                  await submission.reply({
                    ephemeral: true,
                ...
                console.error('[chemistrylab] FRQ grading error details:', {
            ...
            try {
              ...
              let explainRes;
              try {
                console.log('[chemistrylab] Trying primary explanation API...');
                explainRes = await axios.post(`${PRIMARY_BASE}/api/gemini/explain`, {
                  question: question.question,
                  event: 'Chemistry Lab',
                  streaming: false
                }, { headers: AUTH_HEADERS });
                console.log('[chemistrylab] Primary explanation API success');
              } catch (primaryErr) {
                console.log('[chemistrylab] Primary explanation API failed, trying fallback:', primaryErr?.response?.status, primaryErr?.response?.data);
                // Try fallback API
                try {
                  console.log('[chemistrylab] Trying fallback explanation API...');
                  explainRes = await axios.post(`${FALLBACK_BASE}/api/gemini/explain`, {
                    question: question.question,
                    event: 'Chemistry Lab',
                    streaming: false
                  }, { headers: AUTH_HEADERS });
                  console.log('[chemistrylab] Fallback explanation API success');
                } catch (fallbackErr) {
                  console.log('[chemistrylab] Fallback explanation API also failed:', fallbackErr?.response?.status, fallbackErr?.response?.data);
                  throw fallbackErr; // Re-throw to be caught by outer catch
                }
              }
              ...
              if (explainRes.data?.data) {
                if (typeof explainRes.data.data === 'string') {
                  ...
                  console.log('[chemistrylab] Explanation API response structure:', JSON.stringify(explainRes.data.data, null, 2));
              ...
              console.error('[chemistrylab] Explanation error details:', {
                status: err?.response?.status,
                statusText: err?.response?.statusText,
                message: err?.message,
                data: err?.response?.data,
          ...
          console.error('[chemistrylab] Button handler error:', innerErr);
          try {
